...
class ContractExtractor:
    ...
    VENDOR_LABEL_RE = re.compile(
        r"(?i)(?:供货方名称|供方名称|供应商名称|供方|供货方|供应商|乙方|卖方|销售方|承包方|seller|supplier|公司名称|单位名称)"
        r"\s*(?:[（(][^\s）)]*[）)])?\s*[:：]?\s*"
    )
    # 显式法律后缀
    ...
    INVALID_SHORT_NAMES: set[str] = {
        ...
        "审批", "经办", "部门", "主管",
        "采购部", "财务部", "技术部",
        "深圳", "广州", "上海", "北京", "东莞", "佛山", "杭州", "宁波",
        "苏州", "南京", "武汉", "成都", "天津", "重庆", "江苏", "浙江",
        "广东", "福建", "厦门", "无锡", "常州", "中山", "珠海", "惠州",
    ...
    def _contains_vendor_label(self, text: str) -> bool:
        ...
        return any(
            label in lowered
            for label in [
                "供货方名称", "供方名称", "供应商名称", "供方", "供货方", "供应商", "乙方", "卖方", "销售方", "承包方",
                "seller", "supplier", "公司名称", "单位名称",
            ]
        )
